...
#
  ...
  def self.build_report(options = nil)
    ...
    slowest_scenarios = getSlowestScenarios(all_scenarios)

    File.open(getParentFolderPath + '/' + @options[:report_path] + '.html', 'w:UTF-8') do |file|
      ...
      @builder.head do
        ...
        @builder.style(type: 'text/css') do
          @builder << File.read(getGemFilesPath('report_builder') + '/vendor/assets/stylesheets/jquery-ui.min.css')
          COLOR.each do |color|
        ...
        @builder.script(type: 'text/javascript') do
          %w(jquery-min jquery-ui.min highcharts highcharts-3d).each do |js|
            @builder << File.read(getGemFilesPath('report_builder') + '/vendor/assets/javascripts/' + js + '.js')
          end
            ...
            all_features.each_with_index do |feature, n|
              ...
              @builder.div do
                @builder.div(id: "feature#{n}") do
                  feature['elements'].each{|scenario| build_scenario(scenario,slowest_scenarios)}
                end
              end
            end
          end
            ...
            all_scenarios.group_by{|scenario| scenario['status']}.each do |data|
              ...
              @builder.div do
                @builder.div(id: data[0]) do
                  data[1].sort_by{|scenario| scenario['name']}.each{|scenario| build_scenario(scenario,slowest_scenarios)}
                end
              end
            end
          end
  ...
  def self.files(path)
    files = if path.is_a? String
              (path =~ /\.json$/) ? [path] : Dir.glob("#{getParentFolderPath}/#{path}/*.json")
            elsif path.nil?
              Dir.glob('*.json')
  ...
  def self.getParentFolderPath
    return File.dirname(File.dirname(File.dirname(File.dirname(__FILE__))))
  end
  ...
  def self.getSlowestScenarios(all_scenarios)
    no_slowest_scenarios = all_scenarios.size*10/100
    no_slowest_scenarios = no_slowest_scenarios == 0 ? 1 : no_slowest_scenarios

    scenarios_duration = Hash.new { |h,k| h[k] = {} }
    all_scenarios.each do |scenario|
      scenarios_duration[scenario['id']] = scenario['duration']
    end
    scenarios_ordered_by_duration = scenarios_duration.sort_by{ |id, duration| duration }.reverse
    slowest_scenarios = Array.new
    for i in 0..no_slowest_scenarios-1
      slowest_scenarios.push (scenarios_ordered_by_duration[i][0])
    end
    return slowest_scenarios
  end

  def self.getGemFilesPath(gem_name)
    spec = Gem::Specification.find_by_name(gem_name)
    gem_lib = spec.gem_dir
    return gem_lib
  end
